...
def monte_carlo_simulation(rf_, div_, iv_, num_steps, noises):
    ...
    noise = noises[:num_steps]

    simulations = ( (mean - cur_div) - ((std_dev ** 2) / 2)) + noise * std_dev
...
for sb, temp in tqdm(sb_data):
    ...
    ivs = temp['iv_mean']
    Edates_ = temp['steps']
    date_model = zip(rfs, divs, ivs, Edates_)

    simulated_results = [
        monte_carlo_simulation(rf_, div_, iv_, ns_, noises)
        for rf_,div_, iv_, ns_ in date_model
    ]
